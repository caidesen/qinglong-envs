// copy from "https://github.com/go-pkgz/routegroup"
...
func Mount(mux *http.ServeMux, basePath string) Router {
...
func (b *Bundle) Mount(basePath string) Router {
	// copy the middlewares to avoid modifying the original
	middlewares := make([]func(http.Handler) http.Handler, len(b.middlewares))
	copy(middlewares, b.middlewares)
	return &Bundle{
...
func (b *Bundle) With(middleware func(http.Handler) http.Handler, more ...func(http.Handler) http.Handler) Router {
...
func (b *Bundle) Route(configureFn func(Router)) { configureFn(b) }
